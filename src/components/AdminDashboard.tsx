...
import { useState, useEffect } from "react"; // <--- IMPORTAMOS ESTO
...
import {
  ...
  Settings,
  Lock,
  Store, // Agregué este icono para decorar
} from "lucide-react";
import { getStoreSettings } from "@/actions/getStoreSettings"; // <--- IMPORTAMOS LA ACCIÓN
...
export default function AdminDashboard() {
  // Estado para guardar el nombre del local
  const [storeName, setStoreName] = useState("BazarOS");
    ...
    <div className="p-8 max-w-7xl mx-auto animate-in fade-in zoom-in duration-300">
      <div className="mb-8 flex items-center gap-3">
        ...
        <div>
          ...
          {loading ? (
            <p className="text-gray-400 animate-pulse h-6 w-48 bg-gray-100 rounded mt-1"></p>
          ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {MODULES.map((mod, idx) => (
          ...
          >
            <div
              className={`w-14 h-14 rounded-xl ${mod.bg} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}
            >
