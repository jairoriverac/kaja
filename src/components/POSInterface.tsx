...
export default function POSInterface() {
  ...
  const loadLogo = async () => {
    try {
      const response = await fetch("/images/kaja-logo.svg");
  ...
  useEffect(() => {
    const initData = async () => {
      const { data: products } = await supabase
        ...
        .order("name");
      if (products) setQuickProducts(products as Product[]);

      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: settings } = await (supabase.from("settings") as any)
        .select("store_name, address, ruc, phone, email")
        .single();
  ...
  const addToCart = (
    ...
    setCart((prev) => {
      const existing = prev.find(
        (i) => i.dbId === product.id && i.price === finalPrice
      );
      if (existing) {
        return prev.map((i) =>
          i.dbId === existing.dbId && i.price === existing.price
  ...
  const handleSaleComplete = () => {
    setCart([]);
    inputRef.current?.focus();
    showToast("¡Venta finalizada y recibo generado!", "success");
  };

  const getProductStyle = (name: string) => {
    const n = name.toLowerCase();
    if (n.includes("b/n") || (n.includes("blanco") && n.includes("negro")))
      return {
        icon: Printer,
        color: "text-gray-700",
        bg: "bg-white border-gray-300 hover:bg-gray-100 hover:border-gray-400 shadow-sm",
      };
    if (n.includes("color"))
      return {
        icon: Printer,
        color: "text-pink-600",
        bg: "bg-pink-50 border-pink-200 hover:bg-pink-100 hover:border-pink-300 shadow-sm",
      };
    if (n.includes("anillado") || n.includes("plastifi"))
      return {
        icon: Layers,
        color: "text-blue-600",
        bg: "bg-blue-50 border-blue-200 hover:bg-blue-100 hover:border-blue-300 shadow-sm",
      };
    if (n.includes("oficio") || n.includes("redac") || n.includes("impre"))
      return {
        icon: FileText,
        color: "text-orange-600",
        bg: "bg-orange-50 border-orange-200 hover:bg-orange-100 hover:border-orange-300 shadow-sm",
      };
    return {
      icon: Zap,
      color: "text-indigo-600",
      bg: "bg-indigo-50 border-indigo-200 hover:bg-indigo-100 hover:border-indigo-300 shadow-sm",
    };
  };
  ...
  if (!isRegisterOpen) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-gray-100 p-6 text-center">
        ...
        <h1 className="text-3xl font-black text-gray-900 mb-2">Caja Cerrada</h1>
        <p className="text-gray-500 max-w-md mb-8">
          Ya se ha realizado el cierre de caja del día de hoy. No se pueden
          registrar más ventas hasta mañana para evitar errores contables.
        </p>
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className="px-6 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-black transition-colors shadow-lg"
          >
    ...
    <div className="flex h-full bg-gray-100 font-sans overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col p-4 pr-2 gap-4 h-full min-w-0 relative">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm flex items-center gap-3 border border-gray-200 shrink-0 relative z-20">
          ...
          {!isNaN(parseInt(searchQuery)) &&
            ...
            searchQuery.length < 5 && (
              <div className="absolute right-4 bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full animate-in zoom-in">
                Modo Cantidad: {searchQuery}x
              </div>
            )}
      ...
      <div className="w-[380px] bg-white border-l border-gray-200 flex flex-col h-full shadow-xl z-10">
        <div className="p-6 border-b border-gray-100 bg-gradient-to-b from-white to-gray-50 relative">
          <div className="absolute top-4 right-4 flex gap-2">
            ...
            <button
              ...
              title="Ingresar Dinero (Cartera/Sueltos)"
            >
              ...
              {orderedQuickProducts.map((prod) => {
                ...
                return (
                  <button
                    key={prod.id}
                    onClick={() => handleProductClick(prod)}
                    className={`flex items-center p-3 border rounded-xl transition-all active:scale-95 shadow-sm ${
                      style.bg
                    } ${
                      isLongName
                        ? "col-span-2 flex-row justify-between px-4"
        ...
        <div className="p-5 border-t border-gray-200 bg-white">
          ...
          >
            <CreditCard className="w-5 h-5" /> <span>COBRAR</span>
          </button>
        </div>
      </div>
